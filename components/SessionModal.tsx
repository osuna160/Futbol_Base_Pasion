...
const calculateDurationInMinutes = (start: string, end: string): number => {
    ...
    if (endDate < startDate) { // Handle overnight case if needed, though unlikely for training
...
const SessionModal: React.FC<SessionModalProps> = ({ isOpen, onClose, date, sessions, onSave, roster }) => {
  ...
  const startEditing = (session: TrainingSession) => {
    ...
    // --- DATA MIGRATION ---
    if (typeof (sessionCopy as any).warmup === 'string') {
        const oldSession = sessionCopy as any;
        sessionCopy.warmup = { text: oldSession.warmup, duration: 15 };
        sessionCopy.mainPart = { text: oldSession.mainPart, duration: 45 };
        sessionCopy.coolDown = { text: oldSession.coolDown, duration: 10 };
        sessionCopy.finalPart = { text: '', duration: 15 };
        if (oldSession.duration) {
            sessionCopy.endTime = addMinutesToTime(oldSession.startTime, oldSession.duration);
        } else {
            sessionCopy.endTime = addMinutesToTime(oldSession.startTime, 90);
        }
    }
    ...
    // --- ATTENDANCE MIGRATION / DEFAULTING ---
    ...
    if (sessionCopy.attendance && Object.keys(sessionCopy.attendance).length > 0) {
        if (Array.isArray((sessionCopy.attendance as any).present)) {
            const oldAtt = sessionCopy.attendance as any;
            oldAtt.present.forEach((id: number) => { finalAttendance[id] = ATTENDANCE_STATUSES.PRESENT; });
            oldAtt.absent.forEach((id: number) => { finalAttendance[id] = ATTENDANCE_STATUSES.ABSENT; });
            oldAtt.justified.forEach((id: number) => { finalAttendance[id] = ATTENDANCE_STATUSES.OTHER; });
        } else {
          ...
          {editingSession ? (
            <div className="bg-gray-900/50 p-4 rounded-lg space-y-4">
              ...
              <div className="space-y-3 p-3 bg-gray-700/50 rounded-lg">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 items-center">
                      <div className="flex items-center gap-2">
                          <label className="text-gray-400">Inicio:</label>
                          <input type="time" value={editingSession.startTime} onChange={e => setEditingSession(s => s ? {...s, startTime: e.target.value} : null)} className="bg-gray-600 p-2 rounded w-full" />
                      </div>
                      <div className="flex items-center gap-2">
                          <label className="text-gray-400">Fin:</label>
                          <input type="time" value={editingSession.endTime} onChange={e => setEditingSession(s => s ? {...s, endTime: e.target.value} : null)} className="bg-gray-600 p-2 rounded w-full" />
                      </div>
                      <div className="text-center sm:text-left">
                          <span className="font-semibold text-white">{totalSessionDuration}</span>
                          <span className="text-gray-400"> min de sesión</span>
                      </div>
                  </div>
              </div>
              ...
              <div className="flex flex-col gap-6">
                ...
                <div className="space-y-4">
                    ...
                    <div className={`p-2 rounded-lg text-center font-semibold ${totalExerciseDuration > totalSessionDuration ? 'bg-red-900 text-red-300' : 'bg-green-900 text-green-300'}`}>
                        Tiempo de ejercicios: {totalExerciseDuration} min / {totalSessionDuration} min
                    </div>
                    ...
                    <div>
                        ...
                        {editingSession.photoDataUrl ? (
                            <div className="relative group">
                                <img src={editingSession.photoDataUrl} alt="Sesión" className="w-full max-h-60 object-contain rounded-lg" />
                                <button
                                    onClick={() => setEditingSession(s => s ? { ...s, photoDataUrl: undefined } : null)}
                            ...
                            <input type="file" accept="image/*" onChange={handleFileUpload} className="w-full text-sm text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-cyan-600 file:text-white hover:file:bg-cyan-700"/>
                        )}
                    </div>
                </div>
                ...
                {/* Attendance Section */}
                <div>
                    <h4 className="font-bold text-lg text-cyan-400 mb-2">Asistencia ({roster.length})</h4>
                    <div className="bg-gray-700 p-2 rounded-md max-h-[40vh] overflow-y-auto">
                        {roster.sort((a,b) => a.number - b.number).map(player => (
                            <div key={player.id} className="flex justify-between items-center p-1.5 hover:bg-gray-600/50 rounded">
                                ...
                                <select
                                    value={editingSession.attendance?.[player.id] || ATTENDANCE_STATUSES.ABSENT}
                                    onChange={(e) => handleAttendanceChange(player.id, e.target.value as AttendanceStatus)}
                                    className="bg-gray-800 text-white rounded p-1 text-sm border-0 focus:ring-2 focus:ring-cyan-500"
                                >
              ...
              <div className="flex justify-end items-center gap-3 pt-3 mt-4 border-t border-gray-700">
                ...
                <button 
                  ...
                  disabled={!editingSession?.objective?.trim()}
                  className="bg-green-600 hover:bg-green-700 font-semibold py-2 px-4 rounded disabled:bg-gray-500 disabled:cursor-not-allowed"
                >
                  Guardar Sesión
                </button>
              </div>
            ...
            <div>
              {localSessions.length === 0 ? (
                <p className="text-gray-400 text-center py-8">No hay sesiones programadas para este día.</p>
              ) : (
                    ...
                    <div key={session.id} className="bg-gray-700 p-3 rounded-lg">
                      <div className="flex justify-between items-start">
                        ...
                        <div className="flex gap-2">
                           <button onClick={() => startEditing(session)} className="text-gray-300 hover:text-white text-sm font-semibold">EDITAR</button>
                           <button onClick={() => handleRemoveSession(session.id)} className="text-red-400 hover:text-red-300"><TrashIcon /></button>
                        </div>
                      </div>
                    </div>
                  ))}
